"""
Notice that Lonami Exo is NOT the author of this code.
This code was taken from the following website:
  > Safe evaluation of math expressions in pure Python
  > http://opensourcehacker.com/2014/10/29/safe-evaluation-of-math-expressions-in-pure-python/

Also see the following StackOverflow question (by the same person):
  > http://stackoverflow.com/q/26505420/4759433

The original author: Alexer / #python.fi

Example usage:
safe_eval_timeout("1 + a + b", dict(a=2, b=3))
"""
...
class CALL_FUNCTION(OpcodeArg):
    ...
    For each keyword argument, the value is on top of the key. Below the keyword parameters,
    the positional parameters are on the stack, with the right-most parameter on top. Below the parameters,
    the function object to call is on the stack. Pops all function arguments, and the function itself off the
...
def safe_eval_timeout(expr, functions_and_constants={}, check_compiling_input=True):
    """ Hardened compile + eval for long running maths.
